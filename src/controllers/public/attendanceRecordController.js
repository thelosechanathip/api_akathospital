...
exports.getSignatureImage = async (req, res) => {
  try {
    ...
    if (!signature_id || isNaN(signature_id)) {
      return res
        .status(400)
        .json({ message: "signature_id ไม่ถูกต้องหรือไม่ได้ระบุ" });
    }
    ...
    if (!signture || !signture.signature_user_token) {
      return res
        .status(404)
        .json({ message: "ไม่พบภาพหรือข้อมูลที่เกี่ยวข้อง!" });
    }
...
exports.searchDateAttendanceRecord = async (req, res) => {
  try {
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
      return msg(res, 400, {
        message: "วันที่ไม่ถูกต้อง กรุณาระบุในรูปแบบ YYYY-MM-DD!",
      });
    }
...
exports.searchAttendanceRecords = async (req, res) => {
  try {
    ...
    const attendanceIdCondition = isNumeric
      ? { attendance_record_id: { equals: parseInt(keyword) } }
      : {};
...
exports.checkIn = async (req, res) => {
  try {
    ...
    if (!req.body.national_id || !req.body.shift_type_id)
      return msg(res, 400, { message: "กรุณากรอกข้อมูลให้ครบถ้วน" });

    if (!req.body.latitude || !req.body.longitude) {
      return msg(res, 400, {
        message: "ไม่ได้รับข้อมูล Latitude และ Longitude",
      });
    }
    ...
    if (!checkUser)
      return msg(res, 404, {
        message: "ไม่มี User นี้อยู่ในระบบ กรุณา Register ก่อนใช้งาน!",
      });
    ...
    if (!fetchNotify.notify_user_token)
      return msg(res, 400, {
        message:
          "ไม่มี TelegramChatId ใน User กรุณาติดต่อ Admin เพื่อเพิ่มข้อมูล!",
      });
    ...
    if (checkDataAttendanceRecord)
      return msg(res, 409, {
        message: "มีข้อมูลซ้ำที่ยังไม่ได้ Check Out กรุณา Check Out ก่อน!",
      });
    ...
    if (!checkShiftTypeId)
      return msg(res, 404, {
        message: "ไม่มีข้อมูลประเภทกะการทำงาน กรุณาเพิ่มข้อมูลก่อน!",
      });

    let fetchDataOneCheckInStatus = null;
    let fetchDataOneShift = null;

    if (req.body.shift_type_id === 1) {
      // เวลาปกติ
      ...
      if (!fetchDataOneShiftResult)
        return msg(res, 400, {
          message: "ไม่พบกะการทำงานที่ตรงกับเวลาปัจจุบัน",
        });
      ...
      if (
        moment().isBefore(
          moment(fetchDataOneShiftResult.shift_starting, "HH:mm")
            .subtract(
              fetchDataOneShiftResult.shift_starting.slice(0, 2) > 12 &&
                moment().hour() < 6
                ? 1
                : 0,
              "days"
            ...
            .hour(
              moment(fetchDataOneShiftResult.shift_starting, "HH:mm").hour()
            )
            .minute(
              moment(fetchDataOneShiftResult.shift_starting, "HH:mm").minute()
            )
        )
      )
        return msg(res, 400, {
          code: 400,
          result: false,
          message: `ยังไม่ถึงเวลากะนี้ กรุณารอเวลา ${fetchDataOneShiftResult.shift_starting.slice(
            0,
            5
          )}น. เป็นต้นไป`,
      ...
      fetchDataOneCheckInStatus = await pm.check_in_status.findFirst({
        where: {
          check_in_status_name:
            timeNow > fetchDataOneShiftResult.shift_late ? "มาสาย" : "เข้างาน",
        },
      ...
      if (timeNow <= fetchDataOneShiftResult.shift_starting)
        return msg(res, 400, {
          code: 400,
          result: false,
          message: `ยังไม่ถึงเวลากะนี้ กรุณารอเวลา ${fetchDataOneShiftResult.shift_starting.slice(
    ...
    if (!fetchDataOneCheckInStatus)
      return msg(res, 400, {
        message: "เกิดข้อผิดพลาดในการดึงข้อมูลกะการทำงานหรือสถานะเช็คอิน",
      });
    ...
    if (!fetchSignature)
      return msg(res, 404, {
        message:
          "User ยังไม่มีลายเซ็น Digital กรุณาเพิ่มลายเซ็น Digital ก่อนใช้งานระบบ!",
      });
...
exports.checkInVerifyOtp = async (req, res) => {
  ...
  try {
    ...
    if (!cachedOtp)
      return msg(res, 400, { message: "OTP หมดอายุหรือไม่ถูกต้อง" });
    ...
    if (err.name === "TokenExpiredError") {
      ...
      for (const record of fetchDataOneAttendanceRecord) {
        const [startHours, startMinutes, startSeconds] = record.starting
          .split(":")
          .map(Number);
        ...
        if (diffInSeconds >= 20) {
          // ลบข้อมูล
          await pm.attendance_records.deleteMany({
            where: {
          ...
          const maxIdResult =
            await pm.$queryRaw`SELECT COALESCE(MAX(attendance_record_id), 0) + 1 AS nextId FROM attendance_records`;

          // รีเซ็ตค่า AUTO_INCREMENT
          await pm.$executeRawUnsafe(
            `ALTER TABLE attendance_records AUTO_INCREMENT = ${maxIdResult[0].nextId}`
          );
        }
      }
...
exports.checkOut = async (req, res) => {
  try {
    ...
    if (!req.body.national_id || !req.body.latitude || !req.body.longitude)
      return msg(res, 400, { message: "กรุณากรอกข้อมูลให้ครบถ้วน!" });
    ...
    if (!checkDataAttendanceRecord)
      return msg(res, 404, { message: "User นี้ไม่มีการ CheckIn เข้าทำงาน!" });
    ...
    if (checkDataAttendanceRecord.shift_id === null) {
      ...
      if (!fetchOneCheckOutStatus)
        return msg(res, 404, {
          message: "ไม่มีข้อมูล (สถานะการออกงาน) กรุณาเพิ่มข้อมูลก่อน!",
        });
        ...
        const updateData_3 = await pm.attendance_records.update({
          where: {
            attendance_record_id:
              checkDataAttendanceRecord.attendance_record_id,
          },
          data: {
            ending: timeNow,
            ending_signature_id: signature_user_id,
